...
def generate_script(topic=None, specific_hook=None, style="curiosity", is_test=False, lang="en"):
    ...
    Generates a 500-600 word script for a 3-4 minute YouTube Video using Google Gemini (New SDK).
    ...
    TASK:
    1. Break script into **{"2 SHORT SCENES TOTAL" if is_test else "40 to 60 MICRO-SCENES MINIMUM"}**.
    ...
    1. **Natural but Catchy Pacing (CRITICAL)**: Write natural, complete, and coherent sentences. DO NOT write confusing fragments or drop words. Sentences should be about 10 to 18 words to keep it fast-paced, but MUST be grammatically complete and make perfect sense. Visual changes happen every 3-5 seconds. Ensure the narrative is engaging enough so people stay for 3 to 4 minutes!
    2. **Scenes Total**: {"ONLY 2 SCENES" if is_test else "40-60 Micro-Scenes Minimum. You MUST break down every single sentence into visual beats. The TOTAL script must contain 500-600 words to ensure a 3-4 minute video."}
    ...
    {{
      ...
      "seo_title": "CLICKBAIT Viral Title for YouTube Video ({lang_name})",
